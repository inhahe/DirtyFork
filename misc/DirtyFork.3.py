...
class Destinations:
  async def check_keys(user): # todo
    return Screens.main
  async def f(user):
    await ansi_cls(user)
    await send(user, "Testing.")
  

do_screen(user, login.screen)

async def login(user):
  async def check_password(handle, password):
    password_tries.append(password)
    if user_in_db["password_hash"] == bcrypt.hashpw(password, user_in_db["password_salt"]):
      ...
      await send(user, lf*2+b"The password you entered is incorrect.", in_input=False)
  handle_tries = []
  password_tries = []
  ...
  if config.login.screen.path:
    ...
    handle = r.read_lines["handle"]
    password = r.read_lines["password"]
  else:
    await send(user, b"Welcome to " + config.bbs_name.encode("utf-8"), in_input=False)
    for x in range(config.login.allowed_attempts):
      await send(lf*2+'lf your handle or "new" to sign up.'+lf*2, in_input=False, drain=True)
      ...
      if user_in_db is None:
        await send(user, lf*2+handle+b" is not a registered user on this system", in_input=False)
        continue
      await send(user, lf*2+b"Enter your password.", in_input=False)
    ...
    return RetVals(status="max_fail", handle_tries=handle_tries, password_tries=password_tries)


async def failed_login_attempt(user):
  user.failed_login_attempts += 1
  if user.failed_login_attempts < config.login.allowed_attempts:
    await show_screen(user, config.login.fail_screen.path)
    n = config.login.fail_screen.inputs.handle
    await read_line(user, n.row, n.col, n.length) 
  return 
...
async def read_line_char(user, row=None, col=None, length=None):    # should we just ignore invalid inputs after null or esc, or should we process them? 
