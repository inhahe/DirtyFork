...
async def do_input_screen(user, dest, r): # todo: make it so the user can cycle through simultaneous inputs using tab, or enter, but when do we return all the values?
  ...
  while True:
    char, code_name = await get_code(user) # get_code will continually eat up characters until they make up a null or esc sequence for some keyboard key. then it will spit out (False, "convenient_name_for_key")
                                           # if it's just one character not starting with esc or null, then it spits out (that_char, False) # i wonder if i should just always spit out the name or char, i don't 
                                           # think it's like it will be possible to receive, say, "ctrl_u" in one iteration of the loop and confuse it with the convenient key name.. 
                                           # but i do want to do buffering.. but i shouldn't do that here, i should do that in the field_input.process_code function. 
