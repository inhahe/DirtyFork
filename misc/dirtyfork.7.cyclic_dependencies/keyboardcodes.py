        ...
        for wheel_n, wheel in enumerate((None, "up", "down")):
          for row in range(1, 256-32):
            for col in range(1, 256-32): 
              ...
              char2 = chr(row+32)
              char3 = chr(col+32)
              keyboard_codes["\x1"+char1+char2+char3] = Key("click", button=button, shift=shift, alt=alt, ctrl=ctrl, wheel=wheel)

# should we do it this way or have a check_code command that algorithmically checks if it's a partial position or click string? we would have to do it that way if we supported 
# mouse dragging, that probably has 5 values
...
# is there a re.startswith()? there should be..

# todo: move the start codes to this file and make a check function for both start codes and matches