...
def process(x, user, door_config, r):
  if not isinstance(x, str):
    if type(x) in (float, int):
      ...
      x = DirtyFork.to_str(bytes)
    else:
      return x
  d = DirtyFork.global_data # warning: don't let users edit your yaml files!
  d.update(DirtyFork.config)
  d.update(get_dict(user, door_config, r))	  
...
async def run(user, door_config, r): # we could just skip the get_dict step and have a process command in here
  ...
  node_dir = os.path.join(DirtyFork.config.doors.base_path,"/temp/node"+node.node)
