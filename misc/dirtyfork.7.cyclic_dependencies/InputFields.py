from DirtyFork import *
class InputFields: # todo: how will I detect mouse clicks in the middle of getting user input keys?
  def __init__(self, user):
    self.input_fields = []
    self.user = user
  async def InputField(self, field_height, field_length, content_length, fg=config.input_fields.fg, bg=config.input_fields.bg, fg_br=config.input_fields.fg_br, bg_br=contfig.input_fields.bg_br,
...
class InputField:
  async def __init__(self, user, field_height, field_length, content_length, parent=None, fg=config.input_fields.fg, bg=config.input_fields.bg, fg_br=config.input_fields.fg_br, bg_br=config.input_fields.bg_br, \
                 fill=config.input_fields.fill, fill_fg=config.input_fields.fill_fg, fill_bg=config.input_fields.fill_bg, fill_fg_br=config.input_fields.fill_fg_br, \
                 fill_bg_br=config.input_fields.fill_bg_br, scroll_vert=False, scroll_horiz=False, insert_mode = True, outline=None, outline_double=False, content=""):
  ...
  async def run(self):
    ...
    if field_height==1: # somewhere in here, there is a missing close-parentheses
      ...
      if self.col_offset+field_length-1==self.user.screen_width:
      ...
      while True:
        ...
        if key == "left":
          ...
          if self.field_pos == field_length-1: # cursor is at the very right
            if self.field_pos+self.content_pos <= len(self.content): # make sure cursor is not at end of self.content already # off by one maybe?
              ...
              if self.insert_mode:
                ...
                await send(self.user, self.content[self.content_pos:self.content_pos+field_length-1] # send normal characters up until the inverted character
                await ansi_color(self.user, self.bg, self.fg, False, False)
          ...
          if self.field_pos==0:
            ...
            if self.insert_mode:
              ...
              await send(self.user, self.content[self.content[conten_pos:self.content_pos+field_length-self.field_pos]) # check: is this accurate?
          ...
          if end_pos==self.field_pos: # if cursor is right at the end of the self.content
            ...
            if self.insert_mode:
              ...
              await await ansi_color(self.user, self.fill_self.bg, self.fill_self.fg)
              await send(self.user, self.fill)
              await ansi_color(self.user, self.fg, self.bg, self.fg_br, self.bg_br)
          else:
          ...
          if self.insert_mode:
            ...
            await await ansi_color(self.user, self.fg, self.bg, self.fg_br, self.bg_br)
            await send(self.user, key)
            if self.field_pos == end_pos: # cursor is one to the right of the self.content, self.fill char is currently inverted
              if self.field_pos < field_length-1: # self.field_pos better not be > field_length-1, so if it's not < then it's ==, in which case we wouldn't send a new inverted self.fill char
                await await ansi_color(self.user, self.fill_self.bg, self.user_self.fill_self.fg, False, False) 
                await send(self.user, self.fill)
                await ansi_left(self.user) 
              ...
              await await ansi_color(self.user, self.bg, self.fg, False, False)
              await send(self.user, key)
              await await ansi_color(self.user, self.fg, self.bg, self.fg_br, self.bg_br)
              await ansi_left(self.user)
