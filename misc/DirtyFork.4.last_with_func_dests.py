...
async def get_dispatch_key(cur_input, key): # todo: am i actually storing my class instantiations anywhere?? i think so? implicitly?
  result = cur_input(key)
  if result.quit:



async def get_input_key(user): # todo: buffer input when it's coming too fast. but what if keyboard_codes are coming in with it? then we can't treat the buffer as a string for more efficiency.
  while True:     
    char = await user.read.read(1) 
    if char in b"\x00\x1b":
      if user.cur_input_code:
        await get_dispatch_key(user.cur_destination.get_input_key, b"dud:"+user.cur_input_code)
      ...
      if combined in keyboardcodes:
        await get_dispatch_key(user.cur_destination.get_input_key, keyboardcodes[combined])        
      if not combined in keyboard_code_starts:
        await get_dispatch_key(user.cur_destination.get_input_key, b"dud:"+user.cur_input_code)
        await get_dispatch_key(user.cur_destination.get_input_key, char)
        user_cur_input_code = ""
...
async def do_destination(user, destination, r): 
  try:
    lacking_keys = destination.white_keys - user.keys
    bad_keys = destination.black_keys & user.keys
    if lacking_keys or bad_keys:
      status = "Missing keys: " + str(lacking_keys) + "Bad keys: " + str(bad_keys)
      return RetVals(next_destination=user.destination_history[-1], status=status)
    ...
    if destination.type == "class":
      ...
      return await do_input_screen(user, destination.target, r)
      ...
      return await modules[destination.target].run(user, destination, r)
  except:
    send(user, "A critical error has occurred: " + "\n\nAttempting to redirect you to , r)
...
async def user_loop(user):
  ...
  while True: # todo: how do we start it off at config.first_screen?
    ...
    r.previous_destination = user.destination_history[-2] if len(user.destination_history) >= 2 else config.main_destination
    r = await do_destination(user, r.next_destination, r) 
    if r.previous_destination in (config.destinations.register, config.destinations.login): 
      r.next_destination = config.main_destination
async def main():
  while True:
