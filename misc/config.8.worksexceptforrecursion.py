...
class Config(dict):
    ...
    def __getitem__(self, key):
        ...
        if not hasattr(self._root, '_resolving'):
...
# -----------------------------
    ...
    def _resolve(self, value, *, resolving=None):
        if isinstance(value, str):
          value = os.path.expandvars(value)

          # Use existing resolving set from root, or start fresh
          if resolving is not None:
              self._root._resolving = resolving
          elif not hasattr(self._root, '_resolving'):
              self._root._resolving = set()
          ...
          try:
              ...
              # Clean up if we started the resolution
              if resolving is not None:
                  self._root._resolving = set()
...
class _DotFormatter(Formatter):
    def __init__(self, *, strict=True, resolving=None):
        self.strict = strict
